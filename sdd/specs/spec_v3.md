...
**NO SIGNAL**), encender la radio (suena una canción `.mp3` con eco de habitación) y **coger objetos**, como
una escopeta de doble cañón colgada en la pared, que pasan al inventario.

Además, **todo el mapa deja de verse plano**: hundimientos, montículos, tierra acumulada, afloramientos
rocosos y micro-relieve en todo el terreno.
...
- Objetos recogibles: **escopeta de doble cañón** y **farol de aceite**, que se añaden al inventario y se ven en primera persona.
- Radio que reproduce una canción `.mp3` proporcionada, con efecto de altavoz antiguo, eco y reverberación de habitación.
...
### Fuera de alcance (v3)
- Disparar la escopeta, munición, daño o enemigos.
...
- Guardado de partida: el estado de puertas, tele, radio y objetos se reinicia al recargar.
...
| RF-351 | **Sala:** sofá de 2–3 plazas, sillón, mesa baja, alfombra, chimenea de piedra (apagada), estantería con libros, cuadros, **televisor de tubo** con antena de cuernos sobre un mueble y **radio de válvulas** de madera. |
...
| RF-361 | Al mirar un objeto interactivo a ≤ 2,2 m (configurable) aparece un aviso pixel art con la acción: `[E] Abrir puerta`, `[E] Sentarse`, `[E] Encender radio`, `[E] Coger escopeta`… |
...
| ID     | Requisito |
|--------|-----------|
| RF-400 | **Escopeta de doble cañón** (cañones recortados de metal gris y culata de madera marrón) colgada en un soporte sobre la chimenea. |
...
| RF-402 | La escopeta tiene icono pixel art de 16×16 en la barra y modelo en primera persona con balanceo al caminar y animación de sacar/guardar (como la antorcha). No dispara en v3. |
...
### 4.12 Radio con música

| ID     | Requisito |
|--------|-----------|
| RF-410 | Radio de válvulas de madera con dial iluminado. `E` la enciende y la apaga. |
| RF-411 | Al encender: clic, dial que se ilumina en ámbar, 1–2 s de estática de sintonización y después la canción entra con un fundido. |
| RF-412 | La canción es un archivo **`.mp3` proporcionado por el equipo** en `assets/audio/` (ruta en `config.js`); se carga la primera vez que se enciende la radio y suena en bucle. |
| RF-413 | **Efecto de radio antigua:** ecualización de altavoz pequeño (paso alto ≈ 180 Hz, paso bajo ≈ 4,5 kHz, realce de medios), saturación suave y una capa leve de crepitado. |
| RF-414 | **Eco y ambiente:** reverberación de habitación (convolución con respuesta al impulso generada por código) y un eco con retardo y realimentación; la mezcla seca/húmeda es configurable. |
| RF-415 | La música es **espacial**: se oye desde la posición de la radio, baja con la distancia y cambia al girar la cabeza. |
| RF-416 | Fuera de la cabaña se oye amortiguada, como música de fondo (más grave y con más reverberación), y todavía más con la puerta cerrada. |
| RF-417 | La radio respeta la pausa (`Esc`) y el silencio (`M`). |
| RF-418 | Si el archivo no existe o no se puede decodificar, la radio solo emite estática y el HUD (`F3`) lo indica, sin errores en la consola. |

### 4.13 Audio de zonas y pasos
...
| RF-423 | **Zona interior:** dentro de la cabaña los pasos y sonidos interiores tienen reverberación de habitación pequeña y los sonidos exteriores (viento, pájaros, agua) se amortiguan (paso bajo y menos volumen), menos con la puerta abierta. |
...
| ID     | Requisito |
|--------|-----------|
| RF-430 | El HUD (`F3`) añade: zona (exterior o interior), superficie bajo el jugador, distancia al riachuelo y volumen del agua, objeto apuntado y estado de la radio (cargando, sonando, sin archivo). |
...
| RV-307 | La pantalla del televisor y el dial de la radio no reciben niebla y se ven brillantes cuando están encendidos. |
| RV-308 | Modelos de primera persona de la escopeta (referencia: escopeta recortada pixel art, metal gris con brillos y madera marrón) y del farol con el mismo tamaño de píxel que la antorcha. |
...
| RNF-304 | Número de luces fijo: las luces nuevas (lámpara, televisor, relleno interior) existen siempre con intensidad 0 cuando están apagadas; la luz de mano es única y se reconfigura para antorcha o farol. Ninguna proyecta sombras. |
| RNF-305 | Todos los parámetros nuevos (relieve, riachuelo, agua, cabaña, interacción, audio, radio) en `src/config.js`. |
| RNF-306 | Sin dependencias nuevas: Three.js, canvas y Web Audio. El único recurso binario es el `.mp3` de la radio (recomendado ≤ 10 MB). |
...
│   ├── radio.js             # Estado de la radio, dial y control de la música
│   ├── lamp.js              # Lámpara de aceite interactiva
│   └── pickup.js            # Objetos del mundo que pasan al inventario
├── items/
│   ├── inventory.js         # (ampliado) escopeta y farol en ITEMS, añadir a ranura libre
│   ├── handLight.js         # Luz de mano compartida por antorcha y farol
│   ├── shotgun.js           # Modelo en primera persona e icono
...
│   ├── radioChain.js        # Cadena de efectos: EQ de radio, saturación, reverb, eco, panner
│   ├── sfx.js               # Chirridos, golpes, clics, estática, crujidos
│   └── footsteps.js         # (ampliado) madera, agua, barro, grava
├── levels/
│   ├── meadow.js            # (ampliado) riachuelo, puente, cabaña, caminos, terrainFeatures
│   └── cabinLayout.js       # Distribución interior de la cabaña y objetos interactivos (datos)
└── ui/
...
- **Zonas:** la cabaña declara un volumen interior; `acoustics.js` y la iluminación consultan si el jugador está
  dentro y si la puerta está abierta.
...
- [ ] La radio reproduce el `.mp3` con sonido de radio antigua, eco y reverberación, espacial, y se oye de fondo fuera de la cabaña.
